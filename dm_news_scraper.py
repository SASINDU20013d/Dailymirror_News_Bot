...
def create_session(
    ...
    # Set browser-like headers to reduce likelihood of being blocked
...
def fetch_url_text(url: str, timeout: int = 15) -> str | None:
    """
    Fetch URL text with retries and error handling.
    
    Treats HTTP 403 (Forbidden) as a non-fatal error to avoid crashing the script
    when the upstream site blocks the request. Other HTTP errors are also treated
    as non-fatal to make the scraper more resilient to transient failures.
    ...
    try:
        ...
        # Treat 403 Forbidden as non-fatal - log and return None
        # This prevents GitHub Actions job from failing when upstream blocks us
        if resp.status_code == 403:
            print(f"⚠️  Received HTTP 403 (Forbidden) from {url}. The site may be blocking automated requests.", file=sys.stderr)
            print("⚠️  Skipping this run to avoid failing the job. Will retry on next scheduled run.", file=sys.stderr)
            return None
        
        # Raise for other HTTP errors, but catch below to return None
        resp.raise_for_status()
        return resp.text
        
    except requests.RequestException as exc:
        print(f"⚠️  Failed to fetch {url}: {exc}", file=sys.stderr)
        print("⚠️  Skipping this run. Will retry on next scheduled run.", file=sys.stderr)
        return None
...
def extract_article_links(list_url: str = BREAKING_NEWS_URL) -> List[str]:
    ...
    return extract_article_links_from_html(html, list_url)


def extract_article_links_from_html(html: str, list_url: str = BREAKING_NEWS_URL) -> List[str]:
    """Extract Daily Mirror breaking-news article URLs from HTML content."""
...
def cleanup_old_articles(
    ...
    print(f"🕐 Using UTC-aware cutoff time for cleanup: {cutoff.isoformat()}")
...
def main(argv: List[str]) -> None:
    ...
    # Fetch the breaking news page with error handling
    # If fetch_url_text returns None (e.g., 403 Forbidden or network error),
    # we exit gracefully with code 0 to avoid failing the GitHub Actions job.
    # This allows the workflow to succeed even when upstream temporarily blocks us.
    html = fetch_url_text(BREAKING_NEWS_URL)
    if html is None:
        print("⚠️  Could not fetch breaking news list. Exiting gracefully to avoid job failure.")
        print("⚠️  This is expected if the upstream site is blocking requests. Will retry on next run.")
        sys.exit(0)  # Exit with success to prevent GitHub Actions failure

    try:
        article_links = extract_article_links_from_html(html)
    except Exception as exc:  # noqa: BLE001
        raise SystemExit(f"Failed to parse breaking news list: {exc}") from exc
